...
const store = () => new Vuex.Store({
  ...
  mutations: {
    updateFoodCart (state, payload) {
      state.cartDetails.foodsAdded[payload.index].quantity = payload.quantity
      state.cartDetails.cartFoodIds[payload.index].foodQty = payload.quantity
    },
    addToCart (state, payload) {
      state.cartDetails.foodsAdded.push(payload)
      state.cartDetails.cartFoodIds.push({'foodId': payload.dishKID, 'foodQty': payload.quantity})
    },
  ...
  actions: {
    addToCart({ store, commit}, payload) {
      commit('addToCart', payload)
    },
    UpdateCart ({state, commit}, payload) {
      console.log('FFFFFFFFFF',state)
      for(var i=0;i<state.cartDetails.cartFoodIds.length;i++) {
        if(payload.dishKID == state.cartDetails.cartFoodIds[i].foodId) {
          commit('updateFoodCart', {index: i, quantity: payload.quantity})
          break
        }
      }
    },
